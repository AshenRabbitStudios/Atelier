...
- [x] **N isolated instances.** `AgentManager` is fully N-instance (`create`/`open`/`close`/
  ...
  so it loads its own project `CLAUDE.md`. *Needs human spot-check:* two folders side by side,
...
- [ ] Font-scale control on the chat panel — not yet.
  *(Full P2 begins after P1's rewind item closes.)*

## P0 acceptance — self-check
- [x] `npm run dev` opens a window with one docked chat panel. *(launches; window contents
      need a human spot-check — see below)*
- [~] Typing a prompt streams a response token-by-token. *(streaming pipeline proven via
      probe: text deltas accumulate; needs human spot-check in the actual UI)*
- [~] Fenced code renders highlighted (Shiki), whitespace intact, not garbled. *(code path
      built with plain-`<pre>` fallback; needs human spot-check)*
- [~] Thinking block renders collapsed, expands on click. *(built as `<details>`; spot-check)*
- [~] Tool call shows expandable tool_use row with its result. *(built; spot-check — ask the
      agent to read a file)*
- [~] In-flight response can be interrupted. *(Stop button wired to `Query.interrupt()`; spot-check)*
